...
class User(AbstractBaseUser, PermissionsMixin):
    username = models.CharField(_('username'), max_length=32, unique=True, help_text='Required, 30 characters or fewer, starting with letter, letters, digial number',
        validators = [validators.RegexValidator(r'^[a-zA-Z][a-zA-Z0-9_\.]+$',_('Enter a valid username starting with a-z.' 
        'This value may contain only letters, numbers and underscore characters'),'invalid')],
        error_messages= {
    ...
    email = models.CharField(_('email address'), max_length=70, unique=True, null=True, blank=True)
    phone_number = models.BigIntegerField(_('mobile number'), unique=True, null=True, blank=True, validators=[validators.RegexValidator(r'^989[0-3,0]\d{8}$',_('Enter a valid mobile number'), 'invalid')], 
                                          error_messages= {
            'unique':_('A user with that phone number already exists.'),}
        )
    is_staff = models.BooleanField(_('staff status'), default=False)
    is_active = models.BooleanField(_('active'), default=True, help_text='Designates whether this user should be treated as active.'
...
class UserProfile(models.Model):
    ...
    # phone_number = models.BigIntegerField(_('phone number'), null=True, blank=True, validators=[validators.RegexValidator(r'^989[0-3,0]\d{8}$',_('Enter a valid mobile number'), 'invalid')])
